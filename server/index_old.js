...
const validateWithGuardrails = async (req, res, next) => {
  try {
    ...
    if (isBasicGreeting) {
      // If it's a basic greeting, allow it and generate response
      const result = await genAI.getGenerativeModel({ model: 'tunedModels/sparkdfanalyser-gu3i6lr7rlxr' })
        .generateContent(`You are a friendly educational AI assistant. Respond to this greeting: "${prompt}"`);
      const response = await result.response.text();
      return res.json({ response });
    }

    // For non-greetings, proceed with full evaluation
    const evaluationPrompt = `You are an educational AI assistant. 
    Evaluate if this question complies with educational guidelines.
Consider that basic greetings and conversation starters are allowed.
Only respond with one of these three formats:
1. If greeting or some basic conversation starter, respond with: COMPLIANT
2. If compliant, respond with exactly: COMPLIANT
3. If not compliant, respond with exactly: NOT_COMPLIANT: [reason]
...
// AI endpoints
app.post('/api/ai/validate', authenticateUser, async (req, res) => {
  try {
    const { prompt } = req.body;
    ...
    if (isBasicGreeting) {
      // For greetings, generate a friendly response without full validation
